...
static void BM_ConsecutiveWrite(benchmark::State& state) {
    const size_t n_blocks = state.range(0);
    const size_t block_size = state.range(1);
    const bool use_stdio = state.range(2);
        ...
        while (state.KeepRunning()) {
            ...
            for (std::size_t i = 0; i < n_blocks; ++i) {
                ...
                if (bytes != block_size) {
                    ...
                    compio_close_archive(archive);
                    state.SkipWithError(std::string("compio_write returned ") +
...
BENCHMARK(BM_ConsecutiveWrite)
    ->ArgsProduct({{4096, 65536}, {512, 1024, 4096}, {false, true}})
